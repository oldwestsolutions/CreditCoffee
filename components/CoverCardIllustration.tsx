...
const gradients: Record<Variant, { front: string; mid: string; back: string }> = {
  ...
  wealth: {
    front: "from-slate-800 to-slate-950",
    mid: "from-emerald-800 to-forest-900",
    back: "from-cream-400 to-cream-600",
  },
};
...
export function CoverCardIllustration({ variant }: { variant: Variant }) {
  ...
  return (
    <div className="relative h-44 sm:h-48 flex items-center justify-center select-none pointer-events-none" aria-hidden>
      ...
      />
      <div
        className={`relative w-[82%] max-w-[230px] aspect-[1.586/1] rounded-xl bg-gradient-to-br ${g.front} shadow-2xl ring-1 ring-white/10 overflow-hidden`}
      >
        <div className="absolute inset-0 opacity-[0.07] bg-[repeating-linear-gradient(-45deg,transparent,transparent_6px,rgba(255,255,255,0.4)_6px,rgba(255,255,255,0.4)_7px)]" />
        <div className="absolute top-3 left-3 right-3 flex justify-between items-start">
          <div className="h-7 w-10 rounded bg-white/15" />
          <div className="h-5 w-12 rounded bg-white/10" />
        </div>
        <div className="absolute bottom-4 left-4 right-4 space-y-2">
          <div className="h-2 w-[60%] max-w-[140px] rounded bg-white/20" />
          <div className="h-2 w-[40%] max-w-[100px] rounded bg-white/10" />
        </div>
      </div>
    </div>
  );
